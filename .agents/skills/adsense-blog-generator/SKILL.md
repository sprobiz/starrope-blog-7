...
description: 구글 애드센스 수익화 및 구글 SEO 극상 최적화 블로그 포스트 생성, 고품질 HTML 템플릿 작성, 광고 슬롯 배치, 전면 광고 브릿지 연동 및 자동 발행 스크립트 가이드
---

# 구글 애드센스 & SEO 최적화 고품질 블로그 글 생성 스킬 (Starrope Blog System)

이 스킬은 Starrope 네트워크의 7개 구글 애드센스 수익화 블로그에 구글 SEO 최적화 글을 작성하고, 상위 노출과 광고 수익을 극대화하는 고품질 규격을 자동으로 적용하여 발행하는 표준 지침입니다.
...
## 2. 🎯 고품질 구글 SEO & 상위 노출 작성 규격

1. **타이틀(Title) 규칙**:
   - 검색 유저의 클릭률(CTR)을 높이기 위해 **연도(예: 2026년)**, **타겟 대상(어르신/청년/소상공인 등)**, **핵심 혜택/조건/신청방법** 키워드 조합.
   - 예시: `2026년 노인 장기요양보험 신청자격 등급판정 혜택 본인부담금 총정리`

2. **메타 설명(Meta Description)**:
   - 80~120자 내외로 핵심 정보를 요약하고 검색 유입을 유도하는 문장 구성.

3. **구조화 데이터 수록 (JSON-LD Schema.org)**:
   - `<head>` 태그 내 `Article` 및 `FAQPage` 스키마 데이터를 필수 작성하여 피처드 스니펫 최상단 노출.

4. **10초 요약 표 (Summary Table)**:
   - 서론 직후 `summary-table-container` 영역에 핵심 조건, 혜택, 신청 기간, 기관을 표로 수록.

5. **추천 스니펫 타겟팅 (FAQ Accordion 섹션)**:
   - 글 하단에 자주 묻는 질문(FAQ) 3~4개를 HTML `<details>` 및 `<summary>` 태그 기반 아코디언으로 작성.

6. **E-E-A-T 에디터 프로필 박스 (`author-box`)**:
   - 글 하단에 블로그 주제에 적합한 에디터 프로필 박스를 수록하여 전문성 및 신뢰성 제공.

7. **3,000자 이상의 Deep-Dive 콘텐츠**:
   - 세부 기준 표, 5단계 신청 가이드, 예외 규정 등 풍부하고 실질적인 도움이 되는 콘텐츠 제공.

---

## 3. 💡 애드센스 공통 광고 설정 및 전면 광고 트리거 구조
...
4. **발행/동기화 실행**: `scripts/publish.py` 또는 `index.html` & `sitemap.xml` 업데이트를 통해 새 포스트 카드 자동 등록.
5. **GitHub 저장**: `git add .`, `git commit -m "..."`, `git push origin main` 실행.


8. **FAQ ?꾩퐫?붿뼵 ?꾩닔 ?ы븿 (媛뺤젣 ?ы빆)**:
   - 蹂몃Ц ?섎떒??<details>? <summary> ?쒓렇瑜??ъ슜?섏뿬 吏덈Ц怨??듬?(FAQ) 3?명듃 ?댁긽??諛섎뱶???쎌엯?섏꽭??
   - 寃?됱뿏吏??ㅻ땲???몄텧???꾪빐 ?뚯씪 ?곷떒??援ъ“???곗씠??JSON-LD)??@type: ["Article", "FAQPage"]瑜??ы븿?섏꽭??

9. **?쒖닔 ?띿뒪??3,000???댁긽??Deep-Dive 遺꾨웾 ?섎Т??(泥대쪟 ?쒓컙 洹밸???**:
   - 湲??遺꾨웾???듭?濡??섎━??寃껋씠 ?꾨땲??'?ㅼ쟾 ?곸슜???꾪븳 ?ъ링 媛?대뱶', '?λ떒??鍮꾧탳 遺꾩꽍 ??, '?꾨Ц媛 轅?? ???낆옄?먭쾶 ?좎슜???몃? 紐⑹감瑜?履쇨컻??理쒕???湲멸퀬 ?먯꽭?섍쾶 ?묒꽦?섏꽭??
